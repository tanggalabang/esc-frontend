...
const SubjectDash = (props: Props) => {
  ...
  const PAGE_SIZES = [8, 20, 30, 50, 100];
  ...
  const PAGE_SIZES_SUBJECT = [8, 20, 30, 50, 100];
  ...
  useEffect(() => {
    setInitialRecordsSubject(() => {
      return rowDataSubject?.filter((itemSubject: any) => {
        return itemSubject.name.toLowerCase().includes(searchSubject.toLowerCase());
        // item.class.toLowerCase().includes(search.toLowerCase()) ||
        // item.subject.toLowerCase().includes(search.toLowerCase())
      });
    });
  ...
  return (
    <>
      <div className="flex-1 border-white-light px-0 dark:border-[#1b2e4b]">
        {showLoadingSubject ? (
          ...
          <div>
            <div className="invoice-table">
              ...
              <div className="mb-4 grid grid-cols-1 gap-4 px-5">
                <div>
