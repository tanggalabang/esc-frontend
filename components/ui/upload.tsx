...
//   pdf: 'application/pdf',
//   csv: 'text/csv',
//   mp3: 'audio/mpeg', // Add support for MP3
//   mp4: 'video/mp4', // Add support for MP4
//   // word: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', // Update to the correct MIME type
//   word: 'docx', // Update to accept files with the .docx extension
//   excel: 'application/vnd.ms-excel', // Add support for Excel
//   imgAndPdf: 'image/*,application/pdf',
//   all: 'image/*,application/pdf,text/csv,audio/mpeg,video/mp4,application/msword,application/vnd.ms-excel,text/plain',
// };
// const acceptedFileType = {
//   all: '*/*', // Accept all file types
// };
...
export interface UploadProps extends Omit<React.InputHTMLAttributes<HTMLInputElement>, 'accept' | 'children'> {
  ...
  // accept: 'img' | 'pdf' | 'csv' | 'mp3' | 'mp4' | 'word' | 'excel' | 'imgAndPdf' | 'all'; // Add the new file types here
  // accept: 'all'; // Accept all file types
...
function Upload(
  ...
  return (
    <div className={cn(wrapperClassName)}>
      ...
      <div
        className={cn(inputClasses.base, inputClasses.flex, inputClasses.variant[variant].base, inputClasses.variant[variant].color[color], props.disabled && inputClasses.disabled, className)}
        {...dropzoneRootProps}
      >
        <input ref={ref} title="" type="file" accept={acceptedFileType[accept]} className="absolute top-0 h-full w-full opacity-0 disabled:cursor-not-allowed" {...props} {...dropzoneInputProps} />
        <div className="@2xl:flex-row flex flex-col items-center">
          <UploadIcon className={cn(variant !== 'outline' && !props.disabled ? inputClasses.lightTextColor[color] : 'text-gray-300', iconClassName)} />
          {placeholderText || (
            <div className="@5xl::ps-10 @2xl:ps-5 @2xl:text-left pt-2 text-center">
              <h5 className="@2xl:text-base @3xl:mb-3 @3xl:text-lg mb-2 text-sm font-bold text-gray-900">Drop or select file</h5>
              <p className="text-sm leading-relaxed text-gray-900">
                Drop files here or click{' '}
                <span className={cn('font-semibold underline hover:no-underline', variant !== 'outline' && inputClasses.darkTextColor[color], props.disabled && '!text-gray-500')}>browse</span>{' '}
